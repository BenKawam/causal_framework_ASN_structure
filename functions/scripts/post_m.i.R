post_m.i <- function(target_df, # Target values
                     sample, # Posterior samples
                     xmax = 15, # Upper limit of the x axis (natural scale)
                     n_var = 20) # Number of params to sample from 
                     {
  
  target_df <- target_df %>%
    select(dyad, ind_a, ind_b, m_ab) %>%
    rename(g = ind_a, r = ind_b, m = m_ab) %>%
    mutate(direction = 1) %>%
    bind_rows(target_df %>%
                select(ind_a, ind_b, m_ba, dyad) %>%
                rename(g = ind_b, r = ind_a, m = m_ba) %>%
                mutate(direction = 2)) %>%
    filter(dyad < (n_var + 1)) %>%
    group_by(dyad, direction) %>%
    slice(1) %>%
    mutate(param = paste0("m[", g, ",", r, "]"),
  ...
  p1 <- sample %>%
    mutate(direction = ifelse(str_sub(param, start = 1, end = 4) == "m_ab", 1, 2),
...
post_m.i %>% saveRDS("./functions/post_m.i.rds")
